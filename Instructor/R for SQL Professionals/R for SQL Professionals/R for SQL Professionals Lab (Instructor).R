...
#1.0 Planning, setup and environment
	...
	### Start the HTML interface to on-line help (using a web browser available at your machine). Open help this way and briefly explore the features of this facility with the mouse:
		...
		### Remove individual objects no longer needed:
		x <- 10
		X <- 5
		ls()
		rm(x)
		...
		### Show the sample data sets in R:
...
#2.0 Data Structures - http://adv-r.had.co.nz/Data-structures.html
	...
	rm(list = ls())
		...
		#2.2.1 Vectors
		### Vectors have implied distribution during operations. Set up two objects - x and y. Make x a vector from 1 to 10, and y a vector of 2 * x. Show both x and y. Then show the 5th element of y:
        x <- 1:10
		y <- 2 * x
		x
		y
		...
		## You can perform math across arrays apply: array, data set, function
		x <- array(1:20, dim = c(4, 5))
		x
...
#3.0 Data Ingress and Connection Options
	...
	bikesDS2 <- RxOdbcData(sqlQuery = bikesSQL, connectionString = "DSN=test;Uid=<username>;Pwd=<password>;")
...
#4.0 R Functions
	...
	filter() # Like the SQL WHERE Statement - remember, R uses == for equality test. Can also be used with %in%
	...
	myFunction <- function(arg1, arg2, ... )
	{
		...
		costCalc <- function(hours, ppl = 2)
		{
			net.cost <- (hours * ppl)*25
			round(net.cost)
			if (net.cost > 1000) { print("You probably don't want to know. Or have as many meetings.") }
			else {print(net.cost) }
		}
		costCalc(25, 2)
...
########################### EOF: R for SQL Professionals Lab (Instructor).R
